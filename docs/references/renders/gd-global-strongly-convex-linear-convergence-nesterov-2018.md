...
**Source:** [gd-global-strongly-convex-linear-convergence-nesterov-2018.json](../../citations/gd-global-strongly-convex-linear-convergence-nesterov-2018.json)
...
For globally $\mu$-strongly convex, $L$-smooth functions, gradient descent with step size $0 < h \leq 2/(\mu+L)$ achieves global linear convergence: $\|x_k - x^*\|^2 \leq (1 - 2h\mu L/(\mu+L))^k \|x_0 - x^*\|^2$. With the optimal step size $h^* = 2/(\mu+L)$, this gives the rate $\|x_k - x^*\| \leq ((Q_f-1)/(Q_f+1))^k \|x_0 - x^*\|$ where $Q_f = L/\mu$ is the condition number.

## Quote

> Theorem 2.1.15 If $f \in \mathscr{S}_{\mu,L}^{1,1}(\mathbb{R}^n)$ and $0 < h \leq \frac{2}{\mu+L}$, then the Gradient Method generates a sequence $\{x_k\}$ such that $\|x_k - x^*\|^2 \leq \left(1 - \frac{2h\mu L}{\mu+L}\right)^k \|x_0 - x^*\|^2$. If $h = \frac{2}{\mu+L}$, then $\|x_k - x^*\| \leq \left(\frac{Q_f-1}{Q_f+1}\right)^k \|x_0 - x^*\|$, $f(x_k) - f^* \leq \frac{L}{2}\left(\frac{Q_f-1}{Q_f+1}\right)^{2k} \|x_0 - x^*\|^2$, where $Q_f = L/\mu$.
...
![Proof page 1](../extracted-pages/lectures_on_convex_optimization_page_0094.png)
